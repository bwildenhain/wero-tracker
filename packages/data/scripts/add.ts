...
// Ask what type of entity to add
const entityType = await select("What would you like to add?", [
  { value: "bank", label: "Add a bank" },
  { value: "merchant", label: "Add a merchant" },
]);
...
async function addMerchant(data: MerchantsData) {
  ...
  const aliasesInput = await text(
    "Enter merchant aliases separated by comma (or leave empty). These help with search:"
  );
  ...
  const website = await text(
    "Enter the website URL of the merchant (e.g., https://merchant.com):"
  );
